	...
	override function update(elapsed:Float) {
		if (isMenuItem) {
			...
			switch (itemType) {
				...
				case "D-Shape":
					y = FlxMath.lerp(y, (scaledY * 120) + (FlxG.height * 0.45), 0.16 / (openfl.Lib.application.window.frameRate / 60));
					x = FlxMath.lerp(x, Math.exp(Math.abs(scaledY * 0.8)) * 70 + (FlxG.width * 0.10), 0.16 / (openfl.Lib.application.window.frameRate / 60));
				case "Checker":
					y = FlxMath.lerp(y, (scaledY * yMult) + (FlxG.height * 0.40) + yAdd, lerpVal);
