...
class FunkinLua {
	...
	public function new(script:String) {
		...
		set('ratingName', '');
		set('ratingFC', '');
		set('version', TitleState.psychEngineVersion.trim());
		...
		set('resetButton', ClientPrefs.reset);
		...
		Lua_helper.add_callback(lua, "loadSong", function(?name:String = null, ?difficultyNum:Int = -1, ?storyMode:Bool = null) {
            if(name == null || name.length < 1)
                name = PlayState.SONG.song;
            if (difficultyNum == -1)
                difficultyNum = PlayState.storyDifficulty;
            if (storyMode == null) //if nothing then it won't change it 
                storyMode = PlayState.isStoryMode;

            var poop = Highscore.formatSong(name, difficultyNum);
            PlayState.SONG = Song.loadFromJson(poop, name);
            PlayState.storyDifficulty = difficultyNum;
            PlayState.isStoryMode = storyMode; //tells game that Story Mode var = storyMode
		...
		Lua_helper.add_callback(lua, "restartSong", function() {
			PlayState.instance.persistentUpdate = false;
...
class ModchartText extends FlxText
{
	...
	public function new(x:Float, y:Float, text:String, width:Float)
	{
		...
		setFormat(Paths.font("DIN2014Bold.ttf"), 16, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
...
class DebugLuaText extends FlxText
{
	...
	public function new(text:String, parentGroup:FlxTypedGroup<DebugLuaText>, color:FlxColor) {
		...
		setFormat(Paths.font("DIN2014Bold.ttf"), 16, color, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		scrollFactor.set();
		borderSize = 1;
	}
